...
/* ---------- две механики подхода, которые живут прямо в квесте ----------
   Остальные «приёмы интенсивности» из практики про-атлетов убраны: широкой
   аудитории они дают выбор там, где его не должно быть, а натуралу почти
   ничего не добавляют. Суперсет и дроп-сет остались, потому что это не совет,
   а часть схемы подхода: суперсет задаёт порядок движений, дроп-сет — что
   делать с последним подходом. */
export const METHODS = {
  superset: {
    name: "Суперсет", origin: "Кевин Леврон",
...
export const TEMPLATES = {
  U: {
    ...
    slots: [
      { ex: "bench",      alt: "incline-bb",       role: "main" },
      ...
      { ex: "shrug",      alt: "shrug-db",         role: "iso" },
      { ex: "curl-ez",    alt: "incline-curl",     role: "iso",   ss: 2 },
      { ex: "french-db",  alt: "pushdown",         role: "iso",   ss: 2 },
    ],
  },
  L: {
    ...
    slots: [
      ...
      { ex: "legext",        alt: "legext",        role: "iso",   method: "dropset" },
      { ex: "hip-thrust",    alt: "back-ext-45",   role: "iso" },
      { ex: "calf-standing", alt: "calf-seated",   role: "finisher" },
  ...
  F: {
    ...
    slots: [
      ...
      { ex: "calf-seated",  alt: "calf-standing",  role: "finisher" },
...
export function buildExercises(workout, plan = {}) {
  ...
  slots.forEach((slot) => {
    ...
    // механика включается только на своём типе сессии (по умолчанию — объёмная)
    ...
    const fits = ex.tier === 3;   // дроп-сет вешаем только на изоляцию
...
/* ---------- архив прошлых циклов ----------
   ...
   учитывались в прогрессии рабочих весов (id движений совпадают с пулом). */
